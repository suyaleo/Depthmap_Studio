...
Provider-neutral entry for any Agent workspace. Do not open
`~/Developer/agent-rules-kit` just to apply the kit.
...
3. After contracts exist in **this** repo, dry-run then ask once to install the
   adapter used by the current provider. For Cursor:

```bash
"$KIT/adapters/cursor/install/install.sh" . --apply
```

4. With Paseo, keep the current agent as the Primary Agent unless ownership is
   explicitly handed off. Give every writing helper a separate worktree and a
   bounded delegation capsule.
5. Continue work in this workspace. Do not hop back to the kit.
